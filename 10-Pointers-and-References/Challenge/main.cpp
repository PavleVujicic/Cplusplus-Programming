...
********************************************************************/
int * apply_all(const int *const arr1, size_t size1,const int *const arr2, size_t size2){
    int *new_storage_array_ptr {nullptr};
    new_storage_array_ptr = new int[size1 * size2];
    int counter {0};
    
    for(size_t i = 0; i < size2; i++){
        for(size_t j = 0; j < size1; j++){
            *(new_storage_array_ptr + counter) = arr2[i] * arr1[j];
            counter++;
            //cout << "Count number is: " << counter << endl;
        }
    }
    return new_storage_array_ptr;
}
...
int main() {
    ...
    int *results = apply_all(array1, array1_size, array2, array2_size); // need to pass address of apply_all function
    constexpr size_t results_size {array1_size * array2_size};

    cout << "Result: " ;
    print(results, results_size);
    
    delete [] results; // delete alloctaed memory
    cout << endl;
    return 0;
}